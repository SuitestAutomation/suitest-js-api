...
${leaves}`,
	...
	getAppConfigRequest: (appConfigId) => `Getting Application Configuration Data of ${appConfigId}`,
	getAppConfigResponse: (appConfig) => `Application Configuration:
	  name: ${appConfig.name}
	  url: ${appConfig.url}
	  suitestify: ${appConfig.suitestify}
	  domainList: ${appConfig.domainList}
	  variables: ${Object.entries(appConfig.variables).length > 0
		? Object.entries(appConfig.variables).map(([key, value]) => `${key}: ${value}, `)
