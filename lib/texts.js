...
${leaves}`,
	...
	'errorType.queryTimeout': template`Application did not respond for 60 seconds. Executing "${0}". Test session will now close and all remaining Suitest commands will fail. To allow execution of remaining Suitest commands call suitest.startTest() or fix this error.`,
	...
	connectedToDevice: (name) => `Connected to device "${name}"`,
	connectingToDevice: (name) => `Connecting to device "${name}"`,
	...
	chainIsAboutToBeExecuted: (name, toString) => `Executing ${name} - ${toString}`,
