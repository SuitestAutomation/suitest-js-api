...
/**
 * Pair with device
 * @param {Object} instance - SUITEST_API instance and optionally deviceDetails
...
async function pairDevice(
	{deviceDetails, webSockets, authContext, logger, pairedDeviceContext, config},
	deviceId,
	recordingSettings
) {
	...
	if (!deviceDetails) {
		// deviceDetails are defined only when pairDevice is called from bootstrapSession (launcher is used)
		deviceDetails = await getDevicesDetails({authContext}, [{device: deviceId}]);
	}
