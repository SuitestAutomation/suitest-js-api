...
const {authNotAllowed, commandExecuted, sessionClosed, sessionClosing} = require('../texts');
...
async function closeSession() {
	const info = logger.delayed(sessionClosing());

	// don't invalidate tokens in test launcher child processes
	if (process.env[envVars.SUITEST_CLOSE_SESSION] !== 'no') {
