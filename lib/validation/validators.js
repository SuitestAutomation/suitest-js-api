...
const validatePositiveNumberNotNull = (val, name) => {
	if (!Number.isFinite(val) || val <= 0) {
		throwError(name + ' should be positive number');
	}
...
const createStVarOrNumberValidator = (onlyPositiveNumber = false, biggerThanNull = false) => (val, name) => {
	if (!['number', 'string'].includes(typeof val)) {
		throwError(name + ' should be suitest configuration variable or number');
	} else if (typeof val === 'number') {
		if (onlyPositiveNumber && biggerThanNull) {
			(validatePositiveNumberNotNull)(val, name);
		} else {
...
const validateStVarOrPositiveNumberNotNull = createStVarOrNumberValidator(true, true);
