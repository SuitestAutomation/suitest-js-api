...
const createStVarOrNumberValidator = (onlyPositiveNumber = false) => (val, name) => {
	...
	} else if (typeof val === 'string' && !ST_VAR_REGEX.test(val)) {
 ...
 * @throws {SuitestError} if passcode not string or not corresponds to number format ot suitest configuration variable
 * @returns {string}
 */
const validatePasscode = (passcode) => {
	if (typeof passcode !== 'string') {
		throwError('passcode should be string');
	}
	if (Number.isNaN(Number(passcode)) && !ST_VAR_REGEX.test(passcode)) {
		throwError('passcode should have number format or suitest configuration variable');
	}
