...
const toString = data => {
	...
	const text = [
		networkRequestIntro(),
		networkRequestURL(isMatchingString, comparator.val),
	];

	if (request)
		text.push(networkRequestHeaders(request.props, true));

	if (response)
		text.push(networkRequestHeaders(request.props));
...
const toJSON = data => {
	...
	subject.compare = SUBJ_COMPARATOR_M[data.comparator.type];
