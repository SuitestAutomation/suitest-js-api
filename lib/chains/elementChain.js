...
const {PROP_COMPARATOR: PROP_COMPARATOR_M, ELEMENT_PROP, SUBJ_COMPARATOR: SUBJ_COMPARATOR_M} = require('../mappings');
const {VALUE} = require('../constants/element');

const toString = data => {
	const elName = data.selector.apiId || data.selector.css || data.selector.xpath || (data.selector.video && 'video');
	let out = element(elName);

	if (data.comparator) {
		switch (data.comparator.type) {
			case SUBJ_COMPARATOR.EXIST:
				out = elementExist(elName);
				break;
			case SUBJ_COMPARATOR.MATCH_JS:
				out = elementMatchJS(elName);
				break;
			default:
...
const thenComposer = makeThenComposer(toJSON, processServerResponse(toString), toString, methodToString);
...
const getComposers = (data) => {
	...
	if (!data.isNegated && !data.isClick && !data.isMoveTo && !data.sendText) {
	...
	if (!data.comparator && !data.isClick && !data.isMoveTo && !data.sendText) {
		output.push(matchJSComposer, existComposer, matchComposer, matchRepoComposer);
	}
