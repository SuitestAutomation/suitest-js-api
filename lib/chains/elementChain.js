	...
	if (data.comparator) {
		switch (data.comparator.type) {
			...
			case SUBJ_COMPARATOR.MATCH_JS:
				...
				break;
			case SUBJ_COMPARATOR.MATCH_BS:
				out = (data.isNegated ? elementNotMatchBS : elementMatchBS)(elName, data.comparator.val);
				break;
			default: {
				const matches = data.comparator.props.map(
					one => `  ${one.name} ${one.type} ${VALUE_M[one.val] || one.val}`
				);
...
const toJSON = data => {
	...
	if (data.comparator) {
		...
		// match bs
		if (data.comparator.type === SUBJ_COMPARATOR.MATCH_BS) {
			socketMessage.request.condition.val = data.comparator.val;
		}
		// match props
...
const getComposers = (data) => {
	...
	const isMatchesComparator = comparatorType === SUBJ_COMPARATOR_M.MATCH
		...
		|| comparatorType === SUBJ_COMPARATOR_M.MATCH_BS;
	...
	if (!data.comparator && !data.isClick && !data.isMoveTo && !data.sendText) {
		...
		if (!data.isNegated) {
			output.push(matchComposer, matchRepoComposer, matchJSComposer, matchBrightScriptComposer);
		}
	}
