const {omit, compose, clone, isNil} = require('ramda');
...
const videoFactory = require('./videoChain');

const elementFactory = (classInstance) => {
	/**
	...
	const toString = (jsonMessage, nameOnly = false) => {
		const transformSelectorForTitle = (selector) => {
			...
			if (resSelector.hasOwnProperty('ifMultipleFoundReturn')) {
	...
	const deprecatedVideo = util.deprecate(videoFactory(classInstance).video, warnVideoAsElementDeprecation()); // todo can we remove?
