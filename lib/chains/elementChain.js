	...
	if (data.comparator) {
		switch (data.comparator.type) {
			case SUBJ_COMPARATOR.EXIST:
				out = (data.isNegated ? elementNotExist : elementExist)(elName, data.comparator.val);
				break;
			...
			default: {
				const matches = data.comparator.props.map(
					one => `  ${ELEMENT_PROP[one.name]} ${PROP_COMPARATOR_M[one.type]} ${VALUE_M[one.val] || one.val}`
				);
