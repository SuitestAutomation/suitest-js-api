...
const elementFactory = (classInstance, video) => {
	...
	const toString = (jsonMessage, nameOnly = false) => {
		const transformSelectorForTitle = (selector) => {
			...
			if (Reflect.has(resSelector, 'ifMultipleFoundReturn')) {
		...
		if (['assert', 'wait'].includes(def.type) && def.condition.subject.type === 'element') {
			...
			out = elementTap(def.taps[0].type, elName);
		} else if (def.type === 'scroll') {
			const {direction, distance} = def.scroll[0];

			out = elementScroll(elName, direction, distance);
		} else if (def.type === 'swipe') {
			const {direction, distance, duration} = def.swipe[0];

			out = elementSwipe(elName, direction, distance, duration);
