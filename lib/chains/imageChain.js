...
const imageChainFactory = (classInstance) => {
	const toJSON = (data) => {
		...
		if (data.imageData.accuracy) {
			condition.accuracy = data.imageData.accuracy;
		}

		if (data.imageData.imageId) {
			condition.subject.imageId = data.imageData.imageId;
	...
	const thenComposer = makeThenComposer(async(data) => {
		...
		if (data.imageData.url) {
			const imageBuffer = await fetch(data.imageData.url).buffer();
