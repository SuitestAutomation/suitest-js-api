...
const {SUBJ_COMPARATOR_HR, SUBJ_COMPARATOR: SUBJ_COMPARATOR_M} = require('../mappings');
...
const toString = data => {
	...
	switch (data.comparator.type) {
		case SUBJ_COMPARATOR.EXIST:
			return cookieExist(data.cookieName);
		case SUBJ_COMPARATOR.MATCH_JS:
			return cookieMatchJS(data.cookieName);
		default:
			return cookieStringCondition(
				data.cookieName,
				SUBJ_COMPARATOR_HR[data.comparator.type],
				data.comparator.value
			);
	}
};
const methodToString = (data) => `cookie(${data.cookieName || ''})`;
