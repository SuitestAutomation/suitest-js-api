...
const applicationFactory = (classInstance) => {
	const toJSON = data => {
		...
		if (data.isClosed || data.isSuspended) {
			return {
				type: getRequestType(data, false),
				request: {
					type: data.isClosed ? 'closeApp' : 'suspendApp',
				},
			};
		}
	...
	const getComposers = data => {
		...
		if (!isClosed && !isSuspended && !isHasExited && !isSendText) {
			output.push(closeComposer);
		}

		if (!isSuspended && !isClosed && !isHasExited && !isSendText) {
		...
		if (!data.timeout && !isSuspended && !isClosed && !isSendText) {
			output.push(timeoutComposer);
		}

		if (!isHasExited && !isSuspended && !isClosed && !isSendText) {
		...
		} else if (!isHasExited && !isSuspended && !isClosed) {
