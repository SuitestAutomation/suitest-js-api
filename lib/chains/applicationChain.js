...
const applicationFactory = (classInstance) => {
	const toJSON = data => {
		if (isNil(data.sendText)
			&& (!data.comparator || data.comparator.type !== SUBJ_COMPARATOR.HAS_EXITED)
			&& !data.suspendApp
			&& !data.closeApp
		) {
		...
		if (data.closeApp || data.suspendApp) {
			return {
				type: getRequestType(data, false),
				request: {
					type: data.closeApp ? 'closeApp' : 'suspendApp',
				},
			};
		}
	...
	const getComposers = data => {
		...
		const closeApp = !!data.closeApp;
		const suspendApp = !!data.suspendApp;

		if (!data.isAssert) {
			output.push(assertComposer);
		}

		if (!closeApp && !suspendApp && !isHasExited && !isSendText) {
			output.push(closeComposer);
		}

		if (!suspendApp && !closeApp && !isHasExited && !isSendText) {
			output.push(suspendComposer);
		}
		...
		if (!data.timeout && !suspendApp && !closeApp && !isSendText) {
			output.push(timeoutComposer);
		}

		if (!isHasExited && !suspendApp && !closeApp && !isSendText) {
		...
		} else if (!isHasExited && !suspendApp && !closeApp) {
