...
/**
 * @description Attempts to translate specific error messages not related to line execution into more user-friendly descriptions.
 *
 * @param {string} message - The error message to be translated.
 * @return {string} The translated error message if a match is found, otherwise the original message.
 */
const tryTranslateNotLineError = (message) => {
	// selectConfiguration errors
	if (message === 'invalidConfiguration') {
		return texts.invalidConfiguration();
	} else if (message === 'configNotFound') {
		return texts.configNotFound();
	}
	...
	function handleResponse(msg) {
		...
		if (req) {
			if (['query', 'testLine', 'eval', 'takeScreenshot', 'lastScreenshot'].includes(req.contentType)) {
				...
				const message = res.error ? tryTranslateNotLineError(res.error) : texts.unknownError();
