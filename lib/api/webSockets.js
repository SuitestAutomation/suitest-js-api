...
const {getErrorMessage} = require('../utils/socketErrorMessages');
...
function handleResponse(msg) {
	...
	if (req) {
		if (['query', 'testLine', 'eval'].includes(req.contentType)) {
			...
			// TODO: is fatal result possible for any contentType other than
			// 'query', 'testLine', 'eval' ?
			const errorMessage = getErrorMessage({
				chainData: {},
				response: res,
				toString: req.contentType + ' error.',
			});

			if (res.result === 'fatal' && config.dieOnFatalError) {
				console.error(errorMessage);
				process.exit(1);
			}

			req.reject(new SuitestError(errorMessage, SuitestError.UNKNOWN_ERROR));
		}
