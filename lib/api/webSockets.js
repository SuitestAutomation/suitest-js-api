...
function handleResponse(msg) {
	...
	if (req) {
		if (['query', 'testLine', 'eval'].includes(req.contentType)) {
			...
			req.resolve(res);
		} else if (res.error && res.error.match(/device/igm)) {
			// res.error can be Device is disabled, No device is available for test running ...
			// some problem with device reject with appropriate msg
			req.reject(new SuitestError(texts['ws.err.deviceProblem'](res.error), SuitestError.WS_ERROR));
		} else {
