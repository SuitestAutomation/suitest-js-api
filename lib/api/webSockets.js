...
const {getInfoErrorMessage, getNotStartedReasonMessage} = require('../utils/socketErrorMessages');
...
function connect(connectionOps = {}) {
	...
	ws.on('message', msg => {
		...
		logger.debug('Received message:', placeholdEmpty(msg));
...
function handleResponse(msg) {
	...
	if (res.type === 'notRunningReason') {
		logger.log(getNotStartedReasonMessage(res.reason));
	}
...
function send(content) {
	if (ws) {
		...
		logger.debug('Sending message:', placeholdEmpty(msg));
		ws.send(msg);
