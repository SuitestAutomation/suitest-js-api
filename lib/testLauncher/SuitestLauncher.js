...
class SuitestLauncher {
	...
	async runAutomatedSession() {
		...
		try {
			...
			// getDebugPort will just throw error in AUTOMATED mode if --inspect arg is provided
			const debugPort = getDebugPort(this.ownArgv);
			const ipcPort = await setupLauncherIpcServer({
				...this.ownArgv,
		...
		try {
			...
			try {
				...
				const exitCode = await runTestOnDevice(
					this.restArgs, this.ownArgv, deviceDetails, INTERACTIVE, ipcPort, debugPort
				);
