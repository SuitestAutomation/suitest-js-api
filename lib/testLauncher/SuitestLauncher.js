...
class SuitestLauncher {
		...
		try {
			...
			try {
				...
				await new Promise(resolve => queue.start().then(result => {
					...
					Object.keys(devicesLogs).forEach(dId =>
						log.deviceLines(devicesLogs[dId], dId, getDeviceName(device(dId))));
	...
	async runInteractiveSession() {
		...
		try {
			const {
				username, password, orgId, deviceId, appConfigId, logDir,
				inspect, inspectBrk, debugBrk,
			} = this.ownArgv;
				...
				const childProcessCode = await new Promise(resolve => {
					const options = {
						...
						env: {
							...
							[envVars.SUITEST_DEBUG_MODE]: (inspectBrk || inspect || debugBrk) ? 'yes' : 'no',
							FORCE_COLOR: true,
						},
					};
