...
const ETC_DIR = '/etc';
const WIN_DIR = process.platform === 'win32';
const HOME_DIR = WIN_DIR
	? process.env.USERPROFILE
	: process.env.HOME;

const configFiles = [`${APP_NAME}rc.js`,
	`.${APP_NAME}rc.json`,
	`.${APP_NAME}rc.yaml`,
	`.${APP_NAME}rc.yml`,
	`.${APP_NAME}rc.json5`,
...
function directorySearch(directory, foundFiles, configFiles) {
	...
	fs.readdirSync(directory).forEach(file => {
		...
		if (fs.statSync(absolute).isDirectory() && !listOfExclusions.includes(file)) {
			return directorySearch(absolute, foundFiles, configFiles);
		} else if (configFiles.includes(file)) {
			return foundFiles.push(absolute);
		}
	});
}
