		...
		const chainedPromise = chainPromise(async() => {
			...
			if (promiseMap.has(data)) {
				...
				if (beforeSend) {
					...
					const translation = translateLine(
						dataToTranslate, config.logLevel, appContext.context ? appContext.context.configOverride : null
					);
				...
				promise = authContext.authorizeWs(jsonSocketMessage, data.type)
					.then(() => {
						const defaultResponseHandler = processServerResponse(
							logger, config.logLevel, appContext.context ? appContext.context.configOverride : null
						);
