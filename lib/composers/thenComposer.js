		...
		const chainedPromise = chainPromise(async() => {
			...
			if (promiseMap.has(data)) {
				...
				if (beforeSend) {
					...
					const translation = translateLine(dataToTranslate, config.logLevelTestLines);

					if (translation) {
						logger.testLine(translation);
					}
				}
				promise = authContext.authorizeWs(socketMessage, data.type)
					.then(() => webSockets.send(socketMessage).then(result => {
						return callback
							...
							: processServerResponse(logger, config.logLevelTestErrors, config.logLevelTestLines)(
