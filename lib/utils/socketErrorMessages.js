...
function getErrorMessage({errorMap, chainData, toString, response}) {
	...
	return prefix + (errorMap.hasOwnProperty(errorType) ?
		errorMap[errorType](payload) :
		defaultMessage(payload)) + getActualExpectedValues(response, chainData);
}

/**
 * @description Get error type from response object
 * @param {Object} response
 * @returns {string}
 */
function getErrorType(response) {
	switch (true) {
		case response.hasOwnProperty('errorType'):
			return response.errorType;
		case response.hasOwnProperty('executeThrowException'):
			return 'executeThrowException';
		case response.hasOwnProperty('matchJSThrowException'):
			return 'matchJSThrowException';
		default:
			return '';
	}
}
...
const errorMap = {
	...
	executeThrowException: ({response}) => {
		return t['errorType.executeThrowException'](response.executeExceptionMessage);
	},
	matchJSThrowException: ({response}) => {
		return t['errorType.matchJSThrowException'](response.matchJSExceptionMessage);
	},
};
