...
const errorMap = {
	...
	deviceError: ({chainData, toString, response}) => {
		...
		if (response.executeThrowException) {
			return t['errorType.deviceError.jsExpression'](response.executeExceptionMessage);
		}
		if (response.matchJSThrowException) {
			return t['errorType.deviceError.jsExpression'](response.matchJSExceptionMessage);
		}
