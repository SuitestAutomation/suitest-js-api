 ...
 */
const getSnippetLogs = ({testId, definitions, results, level, verbosity, appConfig}, translate = chainUtils.translateLineResult) => {
	...
	return definitionsWithResults.reduce((logs, {def, result}) => {
		logs.push(applyIndent(translate(def, verbosity, result, appConfig)));
 ...
 * @returns {string}
 */
function getErrorMessage({response, jsonMessage, verbosity, snippets, appConfig}) {
	if (response.results && snippets) {
		return getSnippetLogs(
			{verbosity, definitions: snippets, results: response.results, testId: jsonMessage.request.val, level: 1, appConfig},
		);
	}
	...
	return chainUtils.translateLineResult(jsonMessage, verbosity, response, appConfig);
}
