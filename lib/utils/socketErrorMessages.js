...
const errorMap = {
	...
	deviceError: ({chainData, toString, response}) => {
		...
		} else if (response.executeThrowException) {
			message = t['errorType.deviceError.jsExpression'](response.executeExceptionMessage);
		} else if (response.matchJSThrowException) {
			message = t['errorType.deviceError.jsExpression'](response.matchJSExceptionMessage);
		} else {
