...
function getErrorMessage({errorMap, toString, response, jsonMessage}) {
	...
	responseCopy.errorType = normalizeErrorType(responseCopy);
	const payload = {
	...
	if (isErrorFatal(responseCopy))
		prefix = t['prefix.fatal']();
	else if (isWarning(responseCopy))
		prefix = t['prefix.warning']();

	return prefix
		+ (has(responseCopy.errorType, errorMap)
			? errorMap[responseCopy.errorType](payload)
			: defaultMessage(payload))
		+ getActualExpectedValues(responseCopy, jsonMessage);
}
...
function getExpressionActualExpected(res, jsonDef) {
	...
	for (let i = 0; i < res.expression.length; i++) {
		...
		if (path(['message', 'code'], expr) === 'missingProperty') {
			values.push(valTemplate(`${propName}: ${placeholdEmpty(prop.val)}`, false));
			values.push(valTemplate(`${propName}: ${t['actualExpected.missingProperty']()}`, true));
		}
...
function getActualExpectedValues(res, jsonMessage) {
	...
	if (['actualValue', 'expectedValue'].some(prop => has(prop, res))) {
...
const errorMap = {
	...
	adbError: ({response}) => {
		...
		if (code === 'certificateError') {
			return t['errorType.adbError.certificateError']();
		}

		return info && info.reason ? info.reason : t['errorType.adbError']();
	},
	outOfMemory: () => t['errorType.outOfMemory'](),
	invalidPackage: ({response}) => {
		if (responseMessageCode(response) === 'appleTvSimPackageOnDevice') {
			return t['errorType.invalidPackage.appleTvSimPackageOnDevice']();
		}

		return t['errorType.invalidPackage.appleTvDevicePackageOnSim']();
	},
	configuratorError: () => t['errorType.configuratorError'](),
	appleNetworkLogsError: () => t['errorType.appleNetworkLogsError'](),
	appStoreBuild: () => t['errorType.appStoreBuild'](),
	// only for js-api
	notSupportedDriver: () => t['errorType.screenshot.notSupportedDriver'](),
	// only for js-api
	notSupportedIL: () => t['errorType.screenshot.notSupportedIL'](),
	// only for js-api
	timeout: () => t['errorType.screenshot.timeout'](),
	// only for js-api
	generalError: () => t['errorType.screenshot.generalError'](),
	ioError: () => t['errorType.ioError'](),
	netError: () => t['errorType.netError'](),
	sdComponentFailed: () => t['errorType.sdComponentFailed'](),
	MoveTargetOutOfBounds: () => t['errorType.MoveTargetOutOfBounds'](),
	ElementClickIntercepted: () => t['errorType.ElementClickIntercepted'](),
	unsupportedOSVersion: () => t['errorType.unsupportedOSVersion'](),
	targetManagerUnsupportedVersion: () => t['errorType.targetManagerUnsupportedVersion'](),
	systemOutdated: () => t['errorType.systemOutdated'](),
};
