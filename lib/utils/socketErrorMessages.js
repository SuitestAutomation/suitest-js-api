...
function defaultMessage({chainData, toString, response}) {
	...
	return t.unknownServerError(util.inspect(chainData), util.inspect(response));
}
...
const errorMap = {
		...
		switch (code) {
			...
			case 'missingSubject':
				if (chainData.type === 'element')
					return `Element ${JSON.stringify(chainData.selector)} was not found.`;
				break;
			default:
				break;
		}
	...
	invalidRepositoryReference: ({chainData, toString, response}) => {
		const code = responseMessageCode(response);
		const selector = toString(chainData, true);

		switch (code) {
			case 'notExistingElement':
				return t['errorType.invalidRepositoryReference.notExistingElement'](selector);
			case 'unknownProperty':
				return t['errorType.invalidRepositoryReference.unknownProperty'](selector, path(['message', 'property'], response));
			case 'notExistingPlatform':
				return t['errorType.invalidRepositoryReference.notExistingPlatform'](selector);
			default:
				break;
		}
