...
const {arrToString, appendDot} = require('./stringUtils');
...
const testIsNotStarted = 'test is not started';
...
function defaultMessage({chainData, toString, response}) {
	...
	return response.errorType ?
		`${response.errorType}: "${appendDot(toString(chainData))}"${errors}` :
...
function getErrorMessage({errorMap, chainData, toString, response}) {
	return getErrorPrefix(response) +
		getErrorBody({
			errorMap,
			chainData,
			toString,
		...
		getErrorSuffix(response) +
		getActualExpectedValues(response, chainData);
}

/**
 * @description Get error prefix
 * @param {*} [response] webscoket message
 * @returns {string}
 */
function getErrorPrefix(res) {
	return res && res.result === 'fatal' ? t['prefix.fatal']() : '';
}
 ...
 * @returns {string}
 */
function getErrorBody({errorMap, chainData, toString, response}) {
	const payload = {
		chainData,
		toString,
		response,
	...
	return (errorMap.hasOwnProperty(response.errorType)
		...
		: response.error === testIsNotStarted
			// TODO: change when proper errorType is received
			? t['error.testIsNotStarted']()
			: defaultMessage(payload));
}

/**
 * @description Get error suffix
 * @param {*} [response] webscoket message
 * @returns {string}
 */
function getErrorSuffix(res) {
	return res && res.result === 'fatal' && res.error !== testIsNotStarted ? ` ${t['suffix.sessionWillClose']()}` : '';
}
...
function getActualExpectedValues(res, data) {
	...
	if (excludeExpectedValues.includes(res.errorType)) {
		return '';
	}
	...
	return values.length ? ` Failing checks:${arrToString(values, true)}` : '';
}
...
const errorMap = {
	failedStart: ({chainData, toString}) => {
		return appendDot(toString(chainData));
	},
	missingApp: () => {
	...
	internalError: ({chainData, toString}) => {
		return t['errorType.internalError'](appendDot(toString(chainData)));
	},
	...
	serverError: ({chainData, toString}) => {
		return t['errorType.serverError'](appendDot(toString(chainData)));
	},
	invalidCredentials: () => {
	...
	syntaxError: ({chainData, toString, response}) => {
		...
		switch (code) {
			case 'WrongDelay':
				return t['errorType.syntaxError.WrongDelay'](appendDot(toString(chainData)));
			case 'wrongBody':
				return t['errorType.syntaxError.wrongBody'](appendDot(toString(chainData)));
			case 'wrongUrl':
				return t['errorType.syntaxError.wrongUrl'](appendDot(toString(chainData)));
			default:
				return t['errorType.syntaxError'](appendDot(toString(chainData)));
		}
	},
	invalidInput: ({chainData, toString, response}) => {
		...
		switch (code) {
			case 'lineTypeNotSupported':
				return t['errorType.invalidInput.lineTypeNotSupported'](appendDot(toString(chainData)));
			default:
				return t['errorType.invalidInput'](appendDot(toString(chainData)));
		}
	},
	ActionNotAvailable: ({chainData, toString}) => {
		return t['errorType.ActionNotAvailable'](appendDot(toString(chainData)));
	},
	/**
	...
	conditionNotSatisfied: ({chainData, toString}) => {
		return t['errorType.conditionNotSatisfied'](chainData.repeat, appendDot(toString(chainData)));
	},
	...
	illegalButton: ({chainData, toString}) => {
		return t['errorType.illegalButton'](appendDot(toString(chainData)));
	},
	...
	aborted: ({chainData, toString, response}) => {
		...
		return t['errorType.aborted'](appendDot(toString(chainData)));
	},
	queryFailed: ({chainData, toString, response}) => {
		...
		switch (code) {
			...
			case 'applicationError':
				return t['errorType.queryFailed.applicationError'](appendDot(toString(chainData)));
			case 'exprException':
			...
			case 'notFunction':
				return t['errorType.queryFailed.notFunction'](appendDot(toString(chainData)));
			default:
				break;
		}
	...
	noHasLines: ({chainData, toString}) => {
		return t['errorType.noHasLines'](appendDot(toString(chainData)));
	},
	appCrashed: ({chainData, toString}) => {
		return t['errorType.appCrashed'](appendDot(toString(chainData)));
	},
	timeLimitExceeded: ({chainData, toString}) => {
		return t['errorType.timeLimitExceeded'](appendDot(toString(chainData)));
	},
	...
	notExpectedResponse: ({chainData, toString}) => {
		return t['errorType.notExpectedResponse'](chainData.url, appendDot(toString(chainData)));
	},
	noConnection: ({chainData, toString}) => {
		return t['errorType.noConnection'](chainData.url, appendDot(toString(chainData)));
	},
	invalidResult: ({chainData, toString, response}) => {
		...
		switch (code) {
			case 'resultTooLong':
				return t['errorType.invalidResult.resultTooLong'](chainData.url, appendDot(toString(chainData)));
			default:
				break;
		}

		return t['errorType.invalidResult'](chainData.url, appendDot(toString(chainData)));
	},
	lateManualLaunch: () => {
	...
	invalidRepositoryReference: ({chainData, toString, response}) => {
		...
		switch (code) {
			case 'notExistingElement':
				return t['errorType.invalidRepositoryReference.notExistingElement'](appendDot(toString(chainData)));
			case 'unknownProperty':
				return t['errorType.invalidRepositoryReference.unknownProperty'](path(['message', 'property'], response));
			case 'notExistingPlatform':
				return t['errorType.invalidRepositoryReference.notExistingPlatform'](appendDot(toString(chainData)));
			default:
				break;
		}
	...
	NoSuchElement: ({chainData, toString}) => {
		return t['errorType.NoSuchElement'](appendDot(toString(chainData)));
	},
	NoSuchFrame: ({chainData, toString}) => {
		return t['errorType.NoSuchFrame'](appendDot(toString(chainData)));
	},
	UnknownCommand: ({chainData, toString}) => {
		return t['errorType.UnknownCommand'](appendDot(toString(chainData)));
	},
	StaleElementReference: ({chainData, toString}) => {
		return t['errorType.StaleElementReference'](appendDot(toString(chainData)));
	},
	ElementNotVisible: ({chainData, toString}) => {
		return t['errorType.ElementNotVisible'](appendDot(toString(chainData)));
	},
	InvalidElementState: ({chainData, toString}) => {
		return t['errorType.InvalidElementState'](appendDot(toString(chainData)));
	},
	...
	ElementIsNotSelectable: ({chainData, toString}) => {
		return t['errorType.ElementIsNotSelectable'](appendDot(toString(chainData)));
	},
	JavaScriptError: ({chainData, toString}) => {
		return `An error occurred while executing user supplied JavaScript. ${appendDot(toString(chainData))}`;
	},
	XPathLookupError: ({chainData, toString}) => {
		return t['errorType.XPathLookupError'](appendDot(toString(chainData)));
	},
	Timeout: ({chainData, toString}) => {
		return t['errorType.Timeout'](appendDot(toString(chainData)));
	},
	NoSuchWindow: ({chainData, toString}) => {
		return t['errorType.NoSuchWindow'](appendDot(toString(chainData)));
	},
	InvalidCookieDomain: ({chainData, toString}) => {
		return t['errorType.InvalidCookieDomain'](appendDot(toString(chainData)));
	},
	UnableToSetCookie: ({chainData, toString}) => {
		return t['errorType.UnableToSetCookie'](appendDot(toString(chainData)));
	},
	UnexpectedAlertOpen: ({chainData, toString}) => {
		return t['errorType.UnexpectedAlertOpen'](appendDot(toString(chainData)));
	},
	NoAlertOpenError: ({chainData, toString}) => {
		return t['errorType.NoAlertOpenError'](appendDot(toString(chainData)));
	},
	ScriptTimeout: ({chainData, toString}) => {
		return t['errorType.ScriptTimeout'](appendDot(toString(chainData)));
	},
	InvalidElementCoordinates: ({chainData, toString}) => {
		return t['errorType.InvalidElementCoordinates'](appendDot(toString(chainData)));
	},
	IMENotAvailable: () => {
	...
	InvalidSelector: ({chainData, toString}) => {
		return t['errorType.InvalidSelector'](appendDot(toString(chainData)));
	},
	unknownWebDriverKey: () => {
		return t['errorType.unknownWebDriverKey']();
	},
	unfocusableElement: ({chainData, toString}) => {
		return t['errorType.unfocusableElement'](appendDot(toString(chainData)));
	},
	unclickableElement: ({chainData, toString}) => {
		return t['errorType.unclickableElement'](appendDot(toString(chainData)));
	},
	appiumInstanceError: () => {
