...
const {isActive} = require('../testLauncher/repl');
...
function prependStack(stackA, stackB) {
	...
	return stripSuitestCodeFromStack(stackA.replace(
		oldLines,
...
/**
 * Remove stack lines from error stack
 * @param {string} stack
 * @returns {string}
 */
function clearStackLines(stack) {
	return stack.replace(getStackLines(stack), '');
}

/**
 * If repl is active, remove stack lines
 * @param {stack} stack
 * @returns {string}
 */
function normalizeStack(stack) {
	return isActive() ? clearStackLines(stack) : stack;
}

/**
 * If repl is active, remove stack lines, else prepend original stack
 * @param {Error} error
 * @param {string} stack to prepend
 * @returns {Error}
 */
function normalizeError(error, stack) {
	if (isActive()) {
		error.stack = clearStackLines(error.stack);
	} else {
...
const stackTraceWrapper = fn => {
	...
	return async(...args) => {
		try {
			...
			throw normalizeError(error, stack);
		}
	};
};
