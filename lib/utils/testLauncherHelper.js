...
const {AUTOMATED, INTERACTIVE} = require('../constants/modes');
 ...
 */
function attachCtrlCWhenLeavingRepl() {
	logger.debug('Reattaching ctrl+c');
...
function setupReplIpc() {
	function onIpcMessage(chunk) {
		...
		if (msg === REPL_START) {
			...
			attachCtrlCWhenLeavingRepl();
		}
	}

	const server = net.createServer().listen(REPL_IPC_PORT);

	server.on('connection', socket => {
		socket
			.unref()
			.on('data', onIpcMessage)
			.on('error', err => logger.debug('Error on IPC socket:', err));
	});

	server.on('error', err => logger.debug('Error on IPC server', err));
}
