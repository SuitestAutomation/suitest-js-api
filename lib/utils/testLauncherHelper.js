...
const {createWriteStream, mkDirByPathSync} = require('../utils/launcherLoggerHelper');
...
function validateInput(type, args) {
	try {
		validate(
			validators[`TEST_LAUNCHER_${type}`],
...
function writeLogs(deviceId, userArgs, logDir) {
	...
	try {
		if (logDir) {
			...
			launcherLogger._(texts['tl.startLogRecording'](logDir, deviceId));
		}
