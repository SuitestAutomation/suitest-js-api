 ...
 * @param {Error} error
 */
async function handleLauncherError(error) {
...
function writeLogs(deviceId, userArgs, logDir) {
	...
	try {
		if (logDir) {
			...
			launcherLogger._(texts['tl.startLogRecording'](deviceId), deviceId);
			launcherLogger._(`${path.resolve(logDir)}${path.sep}${deviceId}.log`, deviceId);
		}
