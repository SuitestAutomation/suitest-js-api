...
const {writeMessage, startIpc, handleIpcError} = require('./ipcHelper');
 ...
 */
async function launchChild(cmdArgv, ownArgv, options, sessionType, debugPort) {
...
function setupReplIpc() {
	...
	server.on('error', handleIpcError(server));
	server.unref();

	return startIpc(server);
}
 ...
 */
function setupLauncherIpcServer(config) {
	const server = net.createServer();

	server.on('connection', socket => socket.write(writeMessage(messageId.SETUP, {config})));
	server.on('error', handleIpcError(server));
	server.unref();
 ...
 * @returns {Promise<any>}
 */
async function runTestOnDevice(cmdArgv, ownArgv, device, sessionType, ipcPort, debugPort) {
	const {logDir} = ownArgv;
	const options = getChildOptions(device.deviceId, ipcPort);
	const child = await launchChild(cmdArgv, ownArgv, options, sessionType, debugPort);
...
function getChildOptions(deviceId, port) {
	return {
		...
		env: {
			...process.env,
			[envVars.SUITEST_LAUNCHER_PROCESS]: `${deviceId}|${port}`,
			FORCE_COLOR: true,
