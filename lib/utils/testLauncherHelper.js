 ...
 * @param {string} sessionType - Suitest test execution mode (interactive|automated)
 * @returns {string}
 */
function getDebugPort(ownArgw, sessionType) {
	const {inspect, inspectBrk} = ownArgw;
	const debugPort = inspectBrk || inspect;

	/* istanbul ignore if  */
	if (debugPort && sessionType === AUTOMATED) {
...
async function launchChild(cmdArgv, ownArgv, options, sessionType) {
	...
	const debugPort = getDebugPort(ownArgv, sessionType);
 ...
 * @returns {Promise<any>}
 */
async function runTestOnDevice(cmdArgv, ownArgv, device, deviceAccessToken, sessionType) {
	const debugPort = getDebugPort(ownArgv, sessionType);
	const {deviceId} = device;
 ...
 *
 * @param {string} options.deviceId - id of the device
 * @param {string} options.sessionToken - temporary accessToken obtained from server.
 * @param {string} options.appConfigId - app configuration id.
 * @param {boolean} options.isDebugMode - flag determining if launcher should expect debugger attaching.
 * @param {string} options.sessionType - suitest execution mode (interactive|automated)
 * @returns {{shell: true, env: Object}}
 */
function getChildOptions(options) {
	const env = {
		...process.env,

		[envVars.SUITEST_CONFIG]: JSON.stringify({
			...config,
			...options,
