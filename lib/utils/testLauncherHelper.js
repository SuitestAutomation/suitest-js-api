...
async function runTestOnDevice(cmdArgv, ownArgv, device, deviceAccessToken, runMode) {
	const debugPort = getDebugPort(ownArgv, runMode);
	const {deviceId} = device;
	const {logDir, appConfigId, config: configFile} = ownArgv;

	const options = getChildOptions(
		deviceId, deviceAccessToken, appConfigId, debugPort, runMode, configFile
	);
 ...
 * @param {string} runMode - suitest execution mode (interactive|automated)
 * @param {string} configFile - user defined config file
 * @returns {{shell: boolean, env: Object}}
 */
function getChildOptions(deviceId, deviceAccessToken, appConfigId, isDebugMode, runMode, configFile) {
	const env = {
		...process.env,
		...
		[envVars.SUITEST_USER_CONFIG]: configFile,
		FORCE_COLOR: true,
