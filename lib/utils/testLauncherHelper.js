...
function getChildOptions(deviceId, deviceAccessToken, appConfigId, isDebugMode, runMode) {
	const env = {
		...
		[envVars.SUITEST_CONFIG_LOG_DIR]: config.logDir,
		[envVars.SUITEST_CONFIG_DEFAULT_TIMEOUT]: config.defaultTimeout,
		...
		[envVars.SUITEST_CONFIG_TIMESTAMP]: config.timestamp,
		[envVars.SUITEST_LAUNCHER_VERSION]: version,
		[envVars.SUITEST_USER_CONFIG]: config.configFile,
		FORCE_COLOR: true,
