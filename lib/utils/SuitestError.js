...
const {unknownError, invalidConfig, configNotFound} = require('./../texts');
...
class SuitestError extends Error {
	...
	constructor(message, code, response) {
		...
		if (message === SuitestError.INVALID_CONFIG) {
			this.message = invalidConfig();
		} else if (message === SuitestError.CONFIG_NOT_FOUD) {
			this.message = configNotFound();
		}
...
SuitestError.INVALID_CONFIG = 'invalidConfiguration';
SuitestError.CONFIG_NOT_FOUD = 'configNotFound';
