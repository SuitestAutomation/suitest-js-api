 ...
 */
const bootstrapSession = async(suitest, {deviceDetails, configId, presetName}) => {
	try {
...
const connectToIpcAndBootstrapSession = async(suitest) => {
	if (process.env[envVars.SUITEST_CHILD_PROCESS]) {
		const [serializedDeviceInfo, configId, ipcPort] = process.env[envVars.SUITEST_CHILD_PROCESS].split('|');
		const deviceDetails = JSON.parse(serializedDeviceInfo);
