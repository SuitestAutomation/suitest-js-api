...
const {replWarn, recordingUrlInfo} = require('../texts');
...
const bootstrapSession = async(suitest, {deviceId, configId, presetName}) => {
	try {
		if (suitest.config.sessionType === TOKEN) {
			...
			const recordingOption = suitest.config.recordingOption || suitest.config.recording;
			...
			const deviceData = await pairDevice(
				suitest,
				deviceId,
				{recording: suitest.config.recording, webhookUrl: suitest.config.webhookUrl},
			);

			if (recordingOption === 'autostart' && deviceData.recordingUrl) {
				suitest.logger.log(recordingUrlInfo(deviceData.recordingUrl));
			}
