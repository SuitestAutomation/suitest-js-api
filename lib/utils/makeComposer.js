...
const makeModifierComposer = (type, names, applyModifier, {
	...
	const composer = (classInstance, data, chain, makeChain) =>
		names.reduce((acc, name) => ({
			...acc,
			[name]: {
				value: (...args) => {
					const newChain = makeChain(applyModifier(classInstance, data, ...args));

					unregisterParent && classInstance.unusedExpressionWatchers.unregisterLeaf(chain);
					registerClone && classInstance.unusedExpressionWatchers.registerLeaf(newChain, data.stack);
...
const makeMethodComposer = (type, names, callback, {
	...
	const composer = (classInstance, data, chain) =>
		names.reduce((acc, name) => ({
			...acc,
			[name]: {
				value: (...args) => {
					...
					return callback(classInstance, data, ...args);
				},
				configurable,
				writable,
				enumerable,
