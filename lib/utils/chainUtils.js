 ...
 * @param appConfig
 */
const translateLine = (json, logLevel, appConfig) => {
	if (logLevel === 'silent') {
	...
	const line = text.toText(
		translate.translateTestLine({testLine, appConfig}),
...
const translateLineResult = (json, logLevel, lineResult, appConfig) => {
	const testLine = processJsonMessageForToString(json);
	...
	return text.toText(
		translate.translateTestLineResult({testLine, lineResult, appConfig}),
