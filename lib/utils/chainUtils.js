...
const {toText} = require('@suitest/smst-to-text');
const {translateTestLine, translateTestLineResult} = require('@suitest/translate');
...
const translateLineResult = (json, logLevel, lineResult) => {
	...
	return toText(
		translateTestLineResult({testLine, lineResult}),
		{format: true, verbosity: mapLogLevelsToTranslationModule[logLevel]},
	);
};
