 ...
 */
function warnWhenDiffVersions(userVersion, latestVersion) {
	if (!userVersion || !latestVersion) {
		return;
	}

	if (!semver.eq(userVersion, latestVersion)) {
		logger.warn(texts['tl.differentLauncherAndLibVersions'](userVersion, latestVersion));
	}
}

const warnLauncherAndLibHasDiffVersions = () => warnWhenDiffVersions(version, config.latestVersion);
