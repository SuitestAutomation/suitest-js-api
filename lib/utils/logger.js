...
const logger = {
	...
	getAppOutput: (subtype, data) => {
		...
		if (
			config.logLevel === logLevels.silent
			|| subtype === 'time'
			|| subtype === 'assert' && !!data[0] === true // only failed assertions have output
		) {
			return;
		}

		// Handle grouping
		if (subtype === 'group' || subtype === 'groupCollapsed') {
			logger.groupCount++;
		...
		let method = ['timeEnd', 'timeLog', 'trace', 'error', 'assert'].includes(subtype) ? 'log' : subtype;
		...
		let args = data.map(d => {
			// TODO: temporary fix
			// If null, display as strings, because colors.js crashes when tries to color null
			// https://github.com/Marak/colors.js/pull/257
			if (d === null && color) {
				return 'null';
			}
			...
			// Normalize table args
			if (struct === 'table' && val1[1] === null) {
				val1[1] = undefined;
			}
	...
	appOutput: (subtype, data, time) => {
		...
		const {color, method, args} = output;

		// Display prefix line
		console.log(`${stColors.gray(timestamp.formatDate(time) + devicePrefix())} ${stColors.bold('App log:')}`);

		// Indent and group device logs manually, so it does not affect other logs
		console.log();
		for (let i = 0; i <= logger.groupCount; i++) {
			console.group();
		}

		// Display actual device logs
		if (color) {
		...
		// End indentations and groupings
		for (let i = 0; i <= logger.groupCount; i++) {
			console.groupEnd();
		}
		console.log();
