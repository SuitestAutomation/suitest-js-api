...
function createDeviceLogger() {
	...
	const transformStream = new Transform({
		transform(chunk, encoding, callback) {
			const prefix = `${stColors.gray(`${timestamp.formatDate(logTime)} ${devicePrefix()}`)}${stColors.bold('AL:')} `;
	...
	return (method, args, time) => {
		...
		if (appLogger[method]) {
			...
			Raven.captureException(new Error(`Received log method "${method}" instead of "${lowercasedMethod}"`));
