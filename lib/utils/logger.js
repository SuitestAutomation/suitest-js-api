 ...
 * @returns {string}
 */
function decorateEmpty(s) {
	return s.replace(/'\s*'|`\s*`|"\s*"/g, '[EMPTY]');
}
...
const log = method => (...messages) => {
	...
	if (lastLogSource && logSource !== lastLogSource) {
		logLine(decorateEmpty(''), consoleMethod, fontColor, '');
	...
	logMessages.forEach(msg => logLine(decorateEmpty(msg), consoleMethod, fontColor, leftRail));
};
