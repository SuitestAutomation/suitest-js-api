...
const timestampNone = require('../constants/timestamp').none;
...
const log = method => (message, deviceId) => {
	...
	if (config.runningAsLauncher) {
		// when running with REPL, don't prepend timestamp and deviceId to all logs for consistency
		let timestampAndDeviceId = '';

		if (!config.repl) {
			const currentTimestamp = config.timestamp.toLowerCase() === timestampNone
				? ''
				: `[${timestamp(config.timestamp)}] `;

			timestampAndDeviceId = currentTimestamp + devicePrefix(deviceId);
		}

		return console[method.consoleMethod](method.launcherColor
			? method.launcherColor(timestampAndDeviceId + prefix + message)
			: stColors.suit(timestampAndDeviceId) + prefix + message
		);
	}
