...
const methods = {
	success: {
		...
		verbosityName: 'logLevelTestLauncher',
	},
	special: {
		key: 'special',
	...
	testLine: {
		...
		verbosityName: 'logLevelTestLines',
	},
	info: {
		key: 'info',
		color: stColors.gray,
	...
	network: {
		...
		verbosityName: 'logLevelNetworkLogs',
	},
	error: {
		key: 'error',
	...
	testLineError: {
		...
		verbosityName: 'logLevelTestErrors',
	},
	debug: {
		key: 'debug',
		color: colors.blue,
...
const createLogger = (config, pairedDeviceContext) => {
	...
	const log = method => (...messages) => {
		...
		/* istanbul ignore if  */
		if (
			!method.levels.includes(config[method.verbosityName]) ||
			(method.isDebug && !config.logLevelDebug)
		) {
			return;
		}
	...
	const logger = {
		...
		getAppOutput: (subtype, data) => {
			...
			const args = data.map(d => {
				...
				// If element, display as DOM el with textContent if present, do not show inner elements.
				if (struct === 'element') {
					// Text node.
					if (val1.nodeType === 3) {
		...
		appOutput: (subtype, data, time) => {
			...
			if (output && config.logLevelConsoleLogs !== logLevels.silent) {
