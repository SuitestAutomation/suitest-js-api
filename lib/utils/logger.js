...
const logger = {
	 ...
	 * | ['function', string]
	 * | ['table', [any[], any[]]]
	 * | ['trace', StackTrace]} ComplexStruct
	 *
	 ...
	 * @param {number} [timestamp] - required for subtype 'time' | 'timeEnd' | 'timeLog'
	 */
	appOutput: (subtype, data, timestamp) => {
		if (config.logLevel === logLevels.silent) {
			return;
		}

		// Time logs and trace are displayed with console.log and data from device.
		const method = ['time', 'timeEnd', 'timeLog', 'trace'].includes(subtype) ? 'log' : subtype;

		if (typeof data === 'string' || typeof data === 'undefined') {
			data = [data];
		}

		// Timestamps are delivered from device.
		if (['time', 'timeEnd', 'timeLog'].includes(subtype)) {
			const timer = data[0] || 'default';

			if (subtype === 'time') {
				logger.consoleTimestamps[timer] = timestamp;
			} else {
				if (logger.consoleTimestamps[timer]) {
					console.log(`${timer}: ${timestamp - logger.consoleTimestamps[timer]}ms`);
				} else {
					console.warn(`Timer '${timer}' does not exist`);
				}
				if (subtype === 'timeEnd') {
					delete logger.consoleTimestamps[timer];
				}
			}
		} else {
			data.forEach(d => {
				if (!Array.isArray(d)) {
					...
					if (d[1].nodeType === 3) {
						console[method](d[1].nodeValue);
					} else {
						const attrs = d[1].attributes
							? Object.entries(d[1].attributes).map(([attr, val]) => ` ${attr}="${val}"`).join('')
							: '';
						const child = d[1].children && d[1].children[0] && d[1].children[0].nodeType === 3
							? d[1].children[0].nodeValue
							: '...';
						const el = `<${d[1].nodeName}${attrs}>${child}</${d[1].nodeName}>`;

						console[method](el);
					}
				} else if (d[0] === 'trace') {
					// If trace, form stack frames and display each on new line.
					console[method](d[1].map(line => `${line[0]} @ ${line[1]}:${line[2]}`).join('\n'));
				} else {
