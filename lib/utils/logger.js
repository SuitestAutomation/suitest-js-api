...
const createLogger = (config, pairedDeviceContext) => {
	...
	const log = method => (...messages) => {
		...
		if (
			(
				config.consoleLogs
				&& !method.levels.includes(config.consoleLogs)
			)
			|| !method.levels.includes(config.logLevel)
		) {
			return;
		}
	...
	const logger = {
		...
		getAppOutput: (subtype, data) => {
			...
			if (config.consoleLogs === logLevels.silent || config.logLevel === logLevels.silent) {
				return;
			}
