 ...
 * @param logger
 * @param {logLevelTestErrors} logLevelTestErrors level
 * @param {logLevelTestLines} logLevelTestLines level
 * @returns {any}
 */
const processServerResponse = (logger, logLevelTestErrors, logLevelTestLines) =>
	function processServerResponseHandler(res, data, jsonMessage, snippets) {
		...
		const message = getErrorMessage({
			response: res,
			verbosity: isSuccess ? logLevelTestLines : logLevelTestErrors,
			jsonMessage,
			snippets,
		});
		const throwErr = err => {
		...
		if (res.result === 'warning') {
			logger.testLineWarn(infoMessage(''));
		}
		// fatal errors
		if (res.result === 'fatal') {
			logger.testLineError(infoMessage(''));
			throwErr(new SuitestError(message, SuitestError.UNKNOWN_ERROR, responseForError));
		}
		if (isAborted) {
			logger.testLineError(t.executionAborted());
		...
		// testLine success
		if (isSuccess) {
			if (isSnippet && logLevelTestLines) {
				const snippet = getSnippetLogs({
					verbosity: mapLogLevelsToTranslationModule[logLevelTestLines],
		...
		if (isTakeScreenshot && isSuccess) {
			if (data.dataFormat === 'raw') {
				...
				try {
					...
					logger.testLineError(e.message);
					throwErr(e);
				}
			}
