...
const processServerResponse = toString => function processServerResponseHandler(res, data) {
	...
	throw new SuitestError(getGeneralErrorMessage(res));
};
...
/**
 * Create human readable assertion error message from suitest error response
 * @param {Object} res
 * @param {string} text chain specific message (result of calling toString(data))
 * @param {object} data assertion object
 * @returns {String}
 */
function getAssertErrorMessage(res, text, data) {
	...
	if (res.errorType === 'appRunning') {
		...
		if (data.type === 'press') {
			msg += '\n' + texts['err.msg.conditionNotSatisfied'](data.repeat);
		}

		return msg;
	} else if (res.errorType === 'appNotRunning') {
		return texts['err.msg.appNotRunning']();
	}
...
/**
 * Create human readable general error message from suitest error response
 * @param {Object} res
 * @returns {string}
 */
function getGeneralErrorMessage(res) {
	const errorType = path(['result', 'errorType'], res);
	const errorCode = path(['result', 'message', 'code'], res);
	const errorReason = path(['result', 'message', 'info', 'reason'], res);

	if (errorType === 'deviceError' && videoAdapterErrors.includes(errorCode)) {
		return texts.errMsgVideoAdapter(errorReason);
	}

	return res.errorType || res.error;
}
