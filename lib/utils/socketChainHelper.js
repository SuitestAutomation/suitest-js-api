...
const processServerResponse = (logger, verbosity) =>
	function processServerResponseHandler(res, data, jsonMessage, snippets) {
		...
		const message = getErrorMessage({
			response: res,
			verbosity: verbosity?.testErrors || verbosity?.logLevel,
			jsonMessage,
			snippets,
		});
		const throwErr = err => {
		...
		if (isSuccess) {
			if (isSnippet) {
				const snippet = getSnippetLogs({
					...
					verbosity: mapLogLevelsToTranslationModule[verbosity?.testLines] || mapLogLevelsToTranslationModule[verbosity?.logLevel],
