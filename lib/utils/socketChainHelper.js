 ...
 * @returns {any}
 */
const processServerResponse = toString => function processServerResponseHandler(res, data, jsonMessage) {
	...
	if (isTakeScreenshot && isSuccess) {
		if ([undefined, 'raw'].includes(data.formatOrPath)) {
			return res.buffer;
		} else if (data.formatOrPath === 'base64') {
			return res.buffer.toString('base64');
		}
		try {
			fs.writeFileSync(data.formatOrPath, res.buffer);
		} catch (e) {
		...
		logger.error(t.takeScreenshotError() + '\n');
		throwErr(new SuitestError(t.takeScreenshotError(), SuitestError.EVALUATION_ERROR, responseForError));
	}
...
const getTimeoutValue = data => {
	...
	return Number.isFinite(data.timeout) && data.timeout >= 0 ? data.timeout : config.defaultTimeout;
};
