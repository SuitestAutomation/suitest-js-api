...
const processServerResponse = toString => function processServerResponseHandler(res, data) {
	...
	if (res.result === 'warning') {
		logger.warn(getInfoErrorMessage(message, '', res, data.stack));
	}
	// fatal errors
	if (res.result === 'fatal' && !config.continueOnFatalError) {
		logger.error(getInfoErrorMessage(message, '', res, data.stack));
	...
	// eval success
	if (isSuccess(res.result) && res.contentType === 'eval') {
		return true;
	}
	// eval fail
	if (!isSuccess(res.result) && res.contentType === 'eval' && isAssertionError) {
		return false;
	}
	// testLine success
	if (isSuccess(res.result) && res.contentType === 'testLine') {
		return;
	}
	// testLine fail
	if (!isSuccess(res.result) && res.contentType === 'testLine' && isAssertionError) {
		logger.error(getInfoErrorMessage(message, t['prefix.assertionFailed'](), res, data.stack));
...
/**
 * Check if test result should be considered successful
 * @param {string} result
 */
